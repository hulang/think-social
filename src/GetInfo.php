...
final class GetInfo
{
    ...
    public static function qq($token)
    {
        ...
        if ($data['ret'] == 0) {
            ...
            $userInfo['unionid'] = $qq->unionid();
            $userInfo['openid'] = $qq->openid();
            return $userInfo;
        } else {
            throw new \Exception(sprintf('获取腾讯QQ用户信息失败:%s', $data['msg']));
        }
    }
    ...
    public static function weixin($token)
    {
        ...
        if (isset($data['errcode'])) {
            throw new \Exception(sprintf('获取微信用户信息失败:code:%s,msg:%s', $data['errcode'], $data['errmsg']));
        }
        if ($data['openid']) {
    ...
    public static function sina($token)
    {
        ...
        if ($data['id']) {
            ...
            throw new \Exception(sprintf('获取新浪微博用户信息失败:%s', $data['error']));
        }
    }
